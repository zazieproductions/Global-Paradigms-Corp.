        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-cyan-400 font-bold uppercase tracking-widest text-[11px]">
              ZIAA TECHNICAL INVENTORY // 128 VERIFIED RECORDS
            </span>
          </div>
          <h1 className="font-academic text-2xl sm:text-3xl font-bold text-slate-100">
            Acoustic Prototype Catalog (2021–2026)
          </h1>
          <p className="font-academic text-sm text-slate-400 mt-1 max-w-2xl">
            Physical apparatuses fabricated across our Rotterdam cleanroom, high-pressure cavitation tanks, and subterranean cistern arrays. Click any record for complete vector schematics and acoustic specifications.
          </p>
        </div>
      ...
      <div className="border border-slate-800 bg-[#090d14] p-4 rounded-sm space-y-3">
        <div className="flex flex-col sm:flex-row items-center gap-3">
          ...
          <div className="relative flex-1 w-full">
            ...
            <input
              type="text"
              placeholder="Search by apparatus name, code (e.g. ZIAA-PR-003), researcher..."
              value={searchQuery}
          ...
          <div className="flex items-center border border-slate-800 rounded overflow-hidden">
            <button
              ...
              className={`p-2 transition-colors ${viewMode === 'grid' ? 'bg-cyan-600 text-slate-950 font-bold' : 'bg-slate-900 text-slate-400 hover:text-white'}`}
              title="Grid View"
            >
            ...
            <button
              ...
              className={`p-2 transition-colors ${viewMode === 'table' ? 'bg-cyan-600 text-slate-950 font-bold' : 'bg-slate-900 text-slate-400 hover:text-white'}`}
              title="Table View"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1 text-[11px]">
          ...
          <div>
            ...
            >
              ...
              <option value="Level IV (Black Vault)">Level IV (Black Vault)</option>
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="Active Lab Stage">Active Lab Stage</option>
      ...
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredPrototypes.map((proto) => {
            const isBlackVault = proto.clearance.includes('Black Vault');
              ...
              <div
                ...
                className={`p-4 bg-[#090d14] border rounded-sm transition-all flex flex-col justify-between group cursor-pointer ${
                  isBlackVault
                    ...
                    : 'border-slate-800 hover:border-cyan-500/70 hover:bg-[#0c121c]'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between text-[10px] mb-2">
                    <span className="font-bold text-cyan-400 font-mono-code">{proto.code}</span>
                  ...
                  <h3 className="font-academic text-base font-bold text-slate-100 group-hover:text-cyan-300 transition-colors mb-2 leading-snug">
                    {proto.name}
                  </h3>
          ...
          <table className="w-full text-left border-collapse text-[11px]">
            ...
            <tbody>
              {filteredPrototypes.map((proto, idx) => (
                ...
                >
                  <td className="p-3 font-bold text-cyan-400">{proto.code}</td>
                  ...
                  <td className="p-3">
                    <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${
                      proto.clearance.includes('Black Vault')
                        ? 'bg-red-950 text-red-400 border border-red-800'
                  ...
                  <td className="p-3 text-right text-cyan-400 font-bold">VIEW →</td>
                </tr>
              ))}
            </tbody>
          </table>
